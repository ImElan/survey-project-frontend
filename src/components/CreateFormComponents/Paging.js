...
const Paging = ({ totalQuestions, questionsPerPage, pageChangeRequestHandler, currentPage }) => {
    let pageNumbers = [];
            ...
            <Pagination style={{ "justify-content": "center" }}>
                ...
                <Pagination.Ellipsis onClick={() => { showPrevSetOfPages() }} disabled={first == 1} />
                {
                    pageNumbers.map((number) => {
                        return <Pagination.Item onClick={() => pageChangeRequestHandler(number)} active={number == currentPage}>{number}</Pagination.Item>
                    })

                }
                <Pagination.Ellipsis onClick={() => showNextSetOfPages()} disabled={last == totalPages} />
                <Pagination.Next onClick={() => pageChangeRequestHandler(currentPage + 1)} disabled={currentPage == totalPages} />
                <Pagination.Last onClick={() => pageChangeRequestHandler(totalPages)} disabled={currentPage == totalPages} ></Pagination.Last>
            </Pagination>
        </div>
    );
